...
import cleanup_keep_recent_and_old
from datetime import datetime, timezone

class TestCleanupKeepRecentAndOld(unittest.TestCase):
    @patch('cleanup_keep_recent_and_old.boto3.client')
    def test_list_deployments(self, mock_boto_client):
        ...
        result = cleanup_keep_recent_and_old.list_deployments('test-bucket')
    ...
    def test_get_last_modified(self, mock_boto_client):
        ...
        result = cleanup_keep_recent_and_old.get_last_modified('test-bucket', 'deployhash112/')
        self.assertEqual(result, datetime(2024, 6, 20, 10, 10, 10, tzinfo=timezone.utc))

    @patch('cleanup_keep_recent_and_old.boto3.resource')
    def test_delete_deployment(self, mock_boto_resource):
        mock_s3 = MagicMock()
        mock_boto_resource.return_value = mock_s3
        cleanup_keep_recent_and_old.delete_deployment('test-bucket', 'deployhash112/')
